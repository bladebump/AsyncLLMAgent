...
class EnterCondition(BaseModel):
    conditionType: str | None = Field(description="进入条件类型：NONE(无条件)、FRACTION(分值条件)、RANKING(排名条件)", default=None)
    participantLimit: int | None = Field(description="当进入条件为NONE和FRACTION时，表示进入人数和团队数限制，0为不设限制。当进入条件为RANKING时，表示进入排名限制，前N名可以进入", default=None)
...
class CompetitionStage(BaseModel):
    ...
    point: int | None = Field(description="阶段分值，表示此阶段在整个竞赛中的权重", default=None)
    ...
    enterCondition: EnterCondition = Field(description="阶段进入条件，设置进入此阶段的条件", default_factory=EnterCondition)
    mode: str | None = Field(description="阶段类型，可以是CTF、AWD、BTC或THEORY", default=None)

class CTFConfig(BaseModel):
    openType: str | None = Field(description="开题方式，例如'全部开放'、'分批开放'等", default=None)
...
class ScopeData(BaseModel):
    ...
    scoreRate: int | None = Field(description="得分率，影响分值随时间变化的比例", default=None)

class ScopePolicy(BaseModel):
    type: str | None = Field(description="计分方式，例如'动态分值'、'固定分值'等", default=None)
...
class CTFStage(CompetitionStage):
    ...
    answerMode: str | None = Field(description="答题模式，例如'一血有效'、'全部有效'等", default=None)
...
class BTCScorePolicy(BaseModel):
    additional: str | None = Field(description="计分方式的额外说明，用于补充说明BTC阶段的计分规则", default=None)
...
class Competition(BaseModel):
    ...
    stageList: list[CompetitionStage] = Field(description="阶段列表，可以创建CTF（夺旗赛）、AWD（攻防赛）、BTC（闯关赛）、THEORY（理论赛）四种阶段", default=[])
